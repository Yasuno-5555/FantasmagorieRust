...
impl std::fmt::Debug for DeviceBuffer {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Cuda(_) => write!(f, "DeviceBuffer::Cuda"),
            Self::Rocm(_) => write!(f, "DeviceBuffer::Rocm"),
            Self::External(ptr) => write!(f, "DeviceBuffer::External({:#x})", ptr),
            #[cfg(feature = "vulkan")]
            Self::Vulkan(_) => write!(f, "DeviceBuffer::Vulkan"),
            #[cfg(target_os = "macos")]
            Self::Metal(_) => write!(f, "DeviceBuffer::Metal"),
        }
    }
}
        ...
        unsafe {
            ...
            if let Some(alloc) = self.allocation.take() {
                if let Ok(mut lock) = self.allocator.lock() {
                    let _ = gpu_allocator::vulkan::Allocator::free(&mut lock, alloc);
                }
            }
        }
    }
