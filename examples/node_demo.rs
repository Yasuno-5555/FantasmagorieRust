    ...
    event_loop.run(move |event, elwt| {
        ...
        match event {
            ...
            Event::WindowEvent { event: WindowEvent::RedrawRequested, .. } => {
                ...
                // Root container
                let root_box = ctx.r#box().column().width(current_width as f32).height(current_height as f32).padding(20.0).bg(ctx.theme.bg).build();
                ctx.begin(root_box);
                {
                    ctx.text("Node Editor Demo (Right-drag to pan, Scroll to zoom)").font_size(18.0).fg(ctx.theme.accent).build();
                    
                    // Canvas area
                    let canvas = ctx.canvas().size(current_width as f32 - 40.0, current_height as f32 - 80.0).bg(ctx.theme.bg.darken(0.3)).build();
                    ctx.begin(canvas);
                    {
                        // Node 1: Generator
                        let node1 = ctx.node("Generator").pos(100.0, 50.0).size(150.0, 80.0).build();
                        ctx.begin(node1);
                        {
                            let row = ctx.row().build();
                            ctx.begin(row);
                            {
                                ctx.socket("Out", false).color(ColorF::new(1.0, 0.4, 0.4, 1.0)).build();
                            }
                            ctx.end();
                        }
                        ctx.end();
                        ...
                        // Node 2: Filter
                        let node2 = ctx.node("Filter").pos(350.0, 100.0).size(150.0, 100.0).build();
                        ctx.begin(node2);
                        {
                            let row = ctx.row().build();
                            ctx.begin(row);
                            {
                                ctx.socket("In", true).color(ColorF::new(1.0, 0.4, 0.4, 1.0)).build();
                                ctx.socket("Mod", true).color(ColorF::new(0.4, 1.0, 0.4, 1.0)).build();
                                ctx.socket("Out", false).color(ColorF::new(1.0, 1.0, 0.4, 1.0)).build();
                            }
                            ctx.end();
                        }
                        ctx.end();
                        ...
                        // Node 3: Output
                        let node3 = ctx.node("Output").pos(600.0, 80.0).size(130.0, 70.0).build();
                        ctx.begin(node3);
                        {
                            let row = ctx.row().build();
                            ctx.begin(row);
                            {
                                ctx.socket("In", true).color(ColorF::new(1.0, 1.0, 0.4, 1.0)).build();
                            }
                            ctx.end();
                        }
                        ctx.end();
                ...
                fanta_rust::view::renderer::render_ui(root, current_width as f32, current_height as f32, &mut draw_list);
