...
impl Camera {
    ...
    pub fn visible_bounds(&self) -> Rectangle {
        ...
        // Note: Simple AABB for now, might be skewed if rotated
        Rectangle::new(
            p0.x.min(p1.x),
            p0.y.min(p1.y),
            (p1.x - p0.x).abs(),
...
mod tests {
    ...
    fn test_camera_projection() {
        let cam = Camera::new(1000.0, 1000.0);
        let world_pt = Vec2::new(100.0, 100.0);
        
        let screen_pt = cam.world_to_screen(world_pt);
        // Expect (100-0) + 500 = 600
        assert_eq!(screen_pt.x, 600.0);
        assert_eq!(screen_pt.y, 600.0);
        
        let back_to_world = cam.screen_to_world(screen_pt);
        assert!((back_to_world.x - 100.0).abs() < 0.001);
        assert!((back_to_world.y - 100.0).abs() < 0.001);
    }
