...
pub use crate::tracea::kernels::attention::cuda_emitter::FlashAttentionEmitter;
use crate::tracea::core::config::{PipelineConfig, LayoutPolicy};
...
swizzle_enabled=(if config.swizzle_mode != crate::tracea::core::config::SwizzleMode::None { 1 } else { 0 }),
...
impl Emitter for CUDAEmitter {
    fn emit_sync(&mut self, _req: crate::tracea::semantic::transition::SyncRequirement) -> String {
        "__syncthreads();\n".to_string()
    }
    fn generate_from_ir(&self, ir: &UnifiedOpIR) -> String {
        match &ir.op_type {
            ...
            UnifiedOpType::Conv2d { .. } => {
                crate::tracea::emitter::conv::generate_conv(ir)
            }
...
"#)
            ...
            UnifiedOpType::ConvTranspose2d { .. } => {
                crate::tracea::emitter::conv_transpose::generate_conv_transpose(ir)
            }
            UnifiedOpType::LowRankMlp { .. } => {
                crate::tracea::emitter::cuda_low_rank::generate_low_rank_mlp(ir)
            }
        }
    }
}
