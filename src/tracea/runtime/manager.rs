    ...
    fn drop(&mut self) {
        if self.ptr != 0 {
             if let Some(api) = crate::tracea::emitter::rocm_driver::RocmDriverApi::get() {
                 ...
                 // println!("[Runtime] ｧｹ ROCm Buffer freed: 0x{:x}", self.ptr);
             }
        }
    }
}
...
impl RuntimeManager {
    pub fn init(
        ...
        if let Ok(dev) = CudaDevice::new(0) {
            ...
            println!("[Doctor] 泙 CUDA Device Registered.");
        }
        ...
        if let Some(ctx) = &vulkan_ctx {
            devices.insert(DeviceBackend::Vulkan, DeviceHandle {
                ...
                cuda_dev: None,
                vulkan_ctx: Some(Arc::clone(ctx)),
                arch: "vulkan".to_string(), // TODO: Get more specific arch if needed
            });
            println!("[Doctor] 泙 Vulkan Backend Registered (Shared Context).");
        }
        ...
        let id = match backend {
            DeviceBackend::Cuda => {
                ...
                if ptx_res.is_err() {
                     ...
                     println!("[Runtime] 統 Dumped failed source to failed_source.cu");
                }
                ...
                let mut ptx_src = match ptx_res {
                    ...
                    Err(e) => {
                         println!("[Runtime] 笶・JIT Compilation Failed for {}: {}", kernel_name, e);
                ...
                unsafe {
                    ...
                    if res_attr != cudarc::driver::sys::CUresult::CUDA_SUCCESS {
                        println!("[Runtime] 笞・・Failed to set MAX_DYNAMIC_SHARED_SIZE_BYTES: {:?}", res_attr);
                    } else {
    ...
    pub fn save_binary(&self, kernel_name: &str, arch: &str, cubin_path: &str) -> Result<(), String> {
        ...
        println!("[Runtime] 沈 Binary cached to {}", dest_path);
        Ok(())
    }
    ...
    pub fn load_binary(&self, cache_name: &str, kernel_name: &str, arch: &str) -> Result<KernelId, String> {
        ...
        if std::path::Path::new(&cache_path).exists() {
            println!("[Runtime] 噫 AOT Cache Hit: {}", cache_path);
