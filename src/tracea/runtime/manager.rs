...
impl Drop for RocmModule {
    fn drop(&mut self) {
        if !self.0.is_null() {
            if let Some(api) = crate::tracea::emitter::rocm_driver::RocmDriverApi::get() {
...
#[cfg(feature = "vulkan")]
impl std::fmt::Debug for VulkanModule {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("VulkanModule")
...
pub struct DeviceHandle {
    ...
    #[cfg(feature = "vulkan")]
    pub vulkan_dev: Option<Arc<crate::tracea::backend::vulkan::VulkanBackend>>,
    #[cfg(feature = "wgpu")]
    pub wgpu_dev: Option<Arc<crate::backend::wgpu::WgpuBackend>>,
    pub arch: String,
}
...
impl RuntimeManager {
    pub fn init(pref_backend: Option<DeviceBackend>) -> Result<Arc<Self>, String> {
        ...
        if let Some(_) = crate::tracea::emitter::rocm_driver::RocmDriverApi::get() {
        ...
        #[cfg(feature = "vulkan")]
        {
            ...
            if let Ok(vk_backend) = unsafe { crate::tracea::backend::vulkan::VulkanBackend::new() } {
        ...
        let doctor = crate::tracea::doctor::Doctor::global();
    ...
    #[cfg(feature = "wgpu")]
    pub fn init_wgpu(backend: Arc<crate::backend::wgpu::WgpuBackend>) -> Result<Arc<Self>, String> {
        let mut cache = INSTANCE.lock().map_err(|_| "Global Instance Lock Poisoned".to_string())?;
        if let Some(instance) = &*cache {
            return Ok(Arc::clone(instance));
        }
        ...
        let mut devices = HashMap::new();
        devices.insert(DeviceBackend::Wgpu, DeviceHandle {
            backend: DeviceBackend::Wgpu,
            cuda_dev: None,
            #[cfg(feature = "vulkan")]
            vulkan_dev: None,
            #[cfg(feature = "wgpu")]
            wgpu_dev: Some(backend),
            arch: "wgpu".to_string(),
        });
    ...
    #[cfg(feature = "vulkan")]
    pub fn init_vulkan(context: Arc<crate::backend::vulkan::VulkanContext>) -> Result<Arc<Self>, String> {
        let mut cache = INSTANCE.lock().map_err(|_| "Global Instance Lock Poisoned".to_string())?;
        if let Some(instance) = &*cache {
            return Ok(Arc::clone(instance));
        }
        ...
        let mut devices = HashMap::new();
        let vk_backend = unsafe { crate::tracea::backend::vulkan::VulkanBackend::from_context(context)? };
        
        devices.insert(DeviceBackend::Vulkan, DeviceHandle {
            backend: DeviceBackend::Vulkan,
            cuda_dev: None,
            #[cfg(feature = "vulkan")]
            vulkan_dev: Some(Arc::new(vk_backend)),
            #[cfg(feature = "wgpu")]
            wgpu_dev: None,
        ...
        let id = match backend {
            ...
            DeviceBackend::Cuda => {
                ...
                let (jit_code, jit_log) = match &ptx_res {
                 ...
                 self.doctor.on_jit_result(crate::tracea::doctor::JitResultInfo {
                    backend: crate::tracea::doctor::BackendKind::Cuda,
            ...
            DeviceBackend::Rocm => {
                let jit = crate::tracea::emitter::rocm_jit::ROCMJITCompiler::new().ok_or("ROCm JIT API not found")?;
                let binary = jit.compile(source, kernel_name, vec![])?;
                let api = crate::tracea::emitter::rocm_driver::RocmDriverApi::get().ok_or("ROCm Driver API not found")?;
        ...
        match &recorded.handle {
            KernelHandle::Cuda { func, .. } => {
                unsafe {
                    ...
                    if res != cudarc::driver::sys::CUresult::CUDA_SUCCESS { 
                        let msg = format!("{:?}", res);
                        self.doctor.on_kernel_launch(crate::tracea::doctor::KernelLaunchInfo {
                             backend: crate::tracea::doctor::BackendKind::Cuda,
            ...
            KernelHandle::Rocm { func, .. } => {
                let api = crate::tracea::emitter::rocm_driver::RocmDriverApi::get().ok_or("ROCm API not found")?;
                unsafe {
            ...
            KernelHandle::Vulkan { func, .. } => {
                ...
                unsafe {
                    ...
                    device.begin_command_buffer(cmd_buf, &vk::CommandBufferBeginInfo::default().flags(vk::CommandBufferUsageFlags::ONE_TIME_SUBMIT)).map_err(|e| e.to_string())?;
                    ...
                    device.queue_submit(vk_backend.queue, &[vk::SubmitInfo::default().command_buffers(&[cmd_buf])], vk::Fence::null()).map_err(|e| e.to_string())?;
            ...
            #[cfg(feature = "wgpu")]
            KernelHandle::Wgpu { pipeline: _ } => {
                return Err("WGPU Launch not yet implemented in RuntimeManager".to_string());
            }
    ...
    pub fn launch_ttg(
        ...
        ttg: &crate::tracea::runtime::ttg::DeviceTTG,
    ...
    pub fn load_cubin(&self, data: &[u8], name: &str) -> Result<KernelId, String> {
        unsafe {
            ...
            if res != cudarc::driver::sys::CUresult::CUDA_SUCCESS { 
                ...
                self.doctor.on_module_load(crate::tracea::doctor::ModuleLoadInfo {
                     backend: crate::tracea::doctor::BackendKind::Cuda,
    ...
    pub fn load_vulkan(&self, spirv: &[u8], name: &str) -> Result<KernelId, String> {
        ...
        unsafe {
            ...
            let pipeline_res = device.create_compute_pipelines(vk::PipelineCache::null(), &[compute_info], None);
    ...
    pub fn alloc(&self, size_bytes: usize, backend: DeviceBackend) -> Result<BufferId, String> {
        ...
        let buf = match backend {
            ...
            DeviceBackend::Rocm => {
                let api = crate::tracea::emitter::rocm_driver::RocmDriverApi::get().ok_or("No ROCm API")?;
                let mut ptr: u64 = 0;
                unsafe {
            ...
            #[cfg(feature = "wgpu")]
            DeviceBackend::Wgpu => {
                let handle = self.get_device(DeviceBackend::Wgpu)?;
                let wgpu_backend = handle.wgpu_dev.as_ref().ok_or("WGPU device not found")?;
                let buffer = wgpu_backend.device.create_buffer(&wgpu::BufferDescriptor {
                    label: Some("TraceaBuffer"),
                    size: size_bytes as u64,
                    usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC | wgpu::BufferUsages::COPY_DST,
                    mapped_at_creation: false,
                });
                DeviceBuffer::Wgpu(WgpuBuffer {
                    buffer,
                    device: wgpu_backend.device.clone(),
                    queue: wgpu_backend.queue.clone(),
                })
            }
    ...
    pub fn get_device_ptr(&self, id: BufferId) -> Result<u64, String> {
        ...
        match bufs.get(&id).ok_or("No buffer")? {
            ...
            #[cfg(feature = "vulkan")]
            DeviceBuffer::Vulkan(vk_buf) => {
                Ok(vk_buf.buffer.as_raw())
            }
            #[cfg(feature = "wgpu")]
            DeviceBuffer::Wgpu(_) => {
                // WGPU buffers don't have a direct u64 "pointer" in the same sense as CUDA/Vulkan
                Ok(0)
            }
        }
    }
    ...
    pub fn launch_with_policy(
        ...
        kernel_id: KernelId,
        args: Vec<KernelArg>,
        op: &crate::tracea::policy::types::OperatorTopology,
        t_policy: &crate::tracea::policy::types::TilePolicy,
        e_policy: &crate::tracea::policy::types::ExecPolicy,
        epilogue_args: Vec<KernelArg>,
        backend: DeviceBackend,
    ) -> Result<(), String> {
        let layout = crate::tracea::runtime::ttg_builder::TTGBuilder::from_policy(op, t_policy);
        let device_ttg = crate::tracea::runtime::ttg::DeviceTTG::new(self, &layout, backend)?;
