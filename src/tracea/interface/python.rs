...
use crate::tracea::kernels::attention::cuda_emitter::FlashAttentionEmitter; // Import specific emitter for calc logic
use crate::tracea::core::config::PipelineConfig;
use crate::tracea::core::config::LayoutPolicy;
use crate::tracea::core::op::EpilogueOp;
use crate::tracea::core::config::SoftmaxGranularity;
use std::collections::HashMap;
use crate::tracea::core::tuning::{tune_kernel, SearchMode, TunableKernel};
use crate::tracea::kernels::gemm::cpu_adapter::{GemmAdapter, GemmProblem};
use crate::tracea::kernels::attention::cuda_adapter::{Fa2Adapter, Fa2Problem};
use crate::tracea::backend::cpu::CpuBackend;
use crate::tracea::optimizer::benchmark::{Conv2dProblem, NVRTCConvBenchmark};
use crate::tracea::optimizer::{AutoTuner, HardwareProfile};
use std::sync::Mutex;
...
impl PyContext {
    ...
    pub fn attention(
        ...
        let ctx = self;
        let mut request_ctx = crate::tracea::doctor::KernelRequestContext {
            precision_policy: crate::tracea::doctor::PrecisionPolicy::FP16,
            latency_vs_throughput: 0.5,
            allow_fallback: true,
        };

        let decision = crate::tracea::doctor::plan_kernel("fa2", request_ctx);
    ...
    pub fn conv2d(
        ...
        let ir = UnifiedOpIR {
            op_type: UnifiedOpType::Conv2d {
                ...
                layout: crate::tracea::core::config::LayoutPolicy::NHWC,
            },
            precison: "f16".to_string(),
            tiling: config.clone(),
            conv_magic_strategy: None,
        ...
        let (hw_m, hw_s) = crate::tracea::emitter::conv::magic_u32(h_out * w_out);
        let (w_m, w_s) = crate::tracea::emitter::conv::magic_u32(w_out);
        let (sic_m, sic_s) = crate::tracea::emitter::conv::magic_u32(s * c);
        let (c_m, c_s) = crate::tracea::emitter::conv::magic_u32(c);
    ...
    pub fn launch_kernel(&self, py: Python<'_>, id: u64, grid: (u32, u32, u32), block: (u32, u32, u32), smem: u32, args: Vec<PyObject>) -> PyResult<()> {
         ...
         self.runtime.launch(crate::tracea::runtime::manager::KernelId(id), grid, block, smem, k_args).map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e))
    }
    ...
    pub fn optimize_graph(&self, graph: &Bound<'_, PyGraph>, iterations: usize) -> PyResult<()> {
        ...
        for node in &rust_graph.nodes {
            match &node.op {
                crate::tracea::core::graph::Operation::FusedAttention(op) => {
...
pub struct PyDeviceBufferF32 {
    pub id: crate::tracea::runtime::manager::BufferId,
    pub runtime: Arc<RuntimeManager>,
}
#[pymethods]
...
#[derive(Clone)]
pub struct PyGraph {
    pub inner: crate::tracea::core::graph::Graph,
}
...
impl PyGraph {
    #[new]
    pub fn new() -> Self {
        Self { inner: crate::tracea::core::graph::Graph::new() }
    }
...
impl PyTuner {
    ...
    pub fn tune_conv2d(
        ...
        let goal = crate::tracea::optimizer::OptimizationGoal::MaximizeTFLOPS;
