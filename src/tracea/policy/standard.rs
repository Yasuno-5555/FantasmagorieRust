use crate::tracea::policy::engine::{PolicyEngine, PolicyFeedback};
use crate::tracea::policy::types::*;
    ...
    fn choose_tile_shape(&self, device: &DeviceProfile, op: &OperatorTopology) -> [u32; 3] {
        match device.backend {
            ...
            DeviceBackend::Vulkan => {
                // Subgroup-aligned tiles, 32-wide for most GPUs
                [32, 128, 1]
            },
            _ => [32, 32, 1]
        }
    ...
    fn propose(&mut self, ctx: &PolicyContext) -> PolicyDecision {
        ...
        for op in ctx.operators {
            ...
            let backend_hint = match ctx.device.backend {
                ...
                #[cfg(feature = "vulkan")]
                DeviceBackend::Vulkan => BackendExecHint {
                    preferred_block_dim: (64, 1, 1), // Subgroup-aligned
                    max_registers_per_thread: None,
                    use_async_copy: true, // GL_KHR_cooperative_matrix
                },
                _ => BackendExecHint {
