...
impl Graph {
    ...
    pub fn add_fused_gemm(&mut self, m: u32, n: u32, k: u32, epilogue: Vec<crate::tracea::core::op::EpilogueOp>, deps: Vec<usize>) -> usize {
        let id = self.nodes.len();
    ...
    pub fn add_linear(&mut self, in_features: u32, out_features: u32, bias: bool, activation: Vec<crate::tracea::core::op::EpilogueOp>, deps: Vec<usize>) -> usize {
        let id = self.nodes.len();
        let op = crate::tracea::core::op::LinearOp {
             in_features: crate::tracea::core::op::Dim(in_features),
             out_features: crate::tracea::core::op::Dim(out_features),
             bias,
             activation,
        };
        self.nodes.push(Node {
    ...
    pub fn add_attention(&mut self, embed_dim: u32, num_heads: u32, head_dim: u32, causal: bool, deps: Vec<usize>) -> usize {
        ...
        let op = crate::tracea::core::op::AttentionOp {
            embed_dim: crate::tracea::core::op::Dim(embed_dim),
            num_heads: crate::tracea::core::op::Dim(num_heads),
            head_dim: crate::tracea::core::op::Dim(head_dim),
            causal,
        };
        self.nodes.push(Node {
            id,
    ...
    pub fn add_fused_attention(&mut self, b: u32, s: u32, d: u32, h: u32, dh: u32, causal: bool, deps: Vec<usize>) -> usize {
        ...
        self.nodes.push(Node {
            id,
            op: Operation::FusedAttention(crate::tracea::core::op::FusedAttentionOp { b, s, d, h, dh, causal, scale_inv_sqrt_d: true }),
            dependencies: deps,
        });
        id
    }
    ...
    pub fn optimize_fusion(&self) -> Self {
        ...
        for node in &self.nodes {
            ...
            let new_id = match &node.op {
                ...
                Operation::NN(nn) => {
                     match nn {
                        crate::tracea::core::op::NNOp::Linear(lin) => optimized.add_linear(lin.in_features.0, lin.out_features.0, lin.bias, lin.activation.clone(), deps),
                        crate::tracea::core::op::NNOp::Attention(attn) => optimized.add_attention(attn.embed_dim.0, attn.num_heads.0, attn.head_dim.0, attn.causal, deps),
                     }
                }
            };
            id_map.insert(node.id, new_id);
