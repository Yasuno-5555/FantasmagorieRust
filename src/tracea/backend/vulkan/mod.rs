...
impl VulkanBackend {
    ...
    /// Get GLSL primitive definitions for subgroup operations
    pub fn get_primitive_defs() -> String {
        primitives::VulkanPrimitives::get_subgroup_defs()
    }

    pub unsafe fn from_context(ctx: Arc<crate::backend::vulkan::VulkanContext>) -> Result<Self, String> {
        let entry = ctx.entry.clone();
        let instance = ctx.instance.clone();
        let device = ctx.device.clone();
        let p_device = ctx.physical_device;
        let queue = ctx.compute_queue;
        let queue_family_index = ctx.compute_family;
